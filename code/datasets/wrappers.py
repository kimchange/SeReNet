...
@register('sere-fullscanning-wrapper') # if the data have a spatial sampling rate Nnum * Nnum
class SEREFULL(object):
    def __init__(self, dataset, randomSeed=None, inp_size = None, volume_depth = None, augment=False,  \
        sample_views = 13, sample_centerview = None, roi = None, \
            M = 63, Nnum = 13, rand_factor = None, RPN_noise = None, RGN_noise = None, zspacing = None, scanning = None, psfshift = None, config=None):
    ...
    def __getitem__(self, idx):
        ...
        # if self.angle_energy_norm:
        #     lfstack = lfstack / lfstack.mean((1,2), keepdim=True) * lfstack[0,:,:].mean()
        ...
        # resample from lfstack to lfinp, 
        if (self.scanning == 3) and (self.Nnum == 13):
            ii_max = lfstack.shape[1] // self.Nnum
            jj_step = self.Nnum // self.scanning
            start_pos = self.Nnum // 2 - self.scanning // 2 * jj_step
            lfinp = lfstack[:,[start_pos+ii*self.Nnum+jj*jj_step for ii in range(ii_max) for jj in range(self.scanning)],:]
            lfinp = lfinp[:,:,[start_pos+ii*self.Nnum+jj*jj_step for ii in range(ii_max) for jj in range(self.scanning)]]
        else:
            lfinp = F.interpolate(lfstack.unsqueeze(0), size = [lfstack.shape[-2]*self.scanning//self.Nnum, lfstack.shape[-1]*self.scanning//self.Nnum],mode='bilinear',align_corners=False).squeeze(0)
