...
def preDAO(lfstack, defocus=1):
    ...
    with torch.no_grad():
        ...
        for iteration in range(1):
            predLF = ref[0:1,:,:].repeat(lfstack.shape[0],1,1).unsqueeze(0).cuda()
            corr = fftshift(ifft2(fft2(ifftshift(predLF)) * fft2(ifftshift(ref.cuda().flip([-2,-1]).unsqueeze(0) )))).real.squeeze(0)
...
if __name__ == '__main__':
    ...
    if int(args.AOstar):
        ...
        [Si,Sj] = torch.meshgrid(*(torch.arange(Nnum).cuda()-Nnum//2,torch.arange(Nnum).cuda()-Nnum//2),indexing='ij')
        Si = Si.reshape(-1)[input_views]
        Sj = Sj.reshape(-1)[input_views]
        lf_grid = F.affine_grid(torch.tensor([
            [1., 0., 0.],
            [0., 1., 0.]
            ], dtype=torch.float).unsqueeze(0), lfstack.unsqueeze(0).shape, align_corners=False).repeat(lfstack.shape[0],1,1,1).cuda() # D,H,W,2 # 2 means(-1 index, -2 index)
    ...
    for file in files:
        ...
        # depth decomposition 
        inp_all = utils.rolldim_gridsample(lfstack, new_grid.cuda()).unsqueeze(0)
        torch.cuda.synchronize()
        print(f'Depth decomposition time using {time.time() - t0} s')
        
        if inp_size == lfstack.shape[-1]:
            with torch.no_grad():
                ret = model( (inp_all).cuda(), scale)
