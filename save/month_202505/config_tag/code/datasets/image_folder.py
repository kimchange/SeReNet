...
class ImageFolder(Dataset):
    ...
    def __init__(self,  root_path_1, tag = None, split_file=None, split_key=None, first_k=None, last_k=None,
        ...
        filenames = os.listdir(root_path_1)
        ...
        for filename in filenames:
            file = os.path.join(root_path_1, filename)

            if cache == 'none':
                self.files.append(file)

            elif cache == 'bin':
                bin_root = os.path.join(os.path.dirname(root_path_1),
                    '_bin_' + os.path.basename(root_path_1))
