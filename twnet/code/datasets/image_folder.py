...
class ImageFolder(Dataset):
    ...
    def __init__(self,  root_path, tag = None, split_file=None, split_key=None, first_k=None, last_k=None,
        ...
        if split_file is None:
            if tag == 'beads':
                filenames = sorted(os.listdir(root_path),key=lambda x:int(x[5:-8]))
                # print('beads ok')
            elif tag == 'test_No':
                filenames = sorted(os.listdir(root_path),key=lambda x:int(x[7:-4]))
            else:
    ...
    def __getitem__(self, idx):
        ...
        if self.cache == 'none':
            return torch.tensor(  torch.tensor( np.array( tifffile.imread(x), dtype=np.float32) ) )
