...
if figID
    ...
    plot(r0,d(:,1:Ng),'color',[0.2 0.2 0.2 0.5],'linewidth',lnwd/4);
    hold on
    radAv(1) = radAv(2); %for plot 
    radAv(end) = radAv(end-1);
    % plot(linspace(0,1,length(radAv)),linmap(radAv,0,1),'linewidth',lnwd,'color',[1 0 1])
    for n = 1:Ng
        plot(r2,d(:,n+Ng:end),'color',[0 0 (n-1)/Ng],'linewidth',lnwd/4)
    end
    plot(r0,d0,'linewidth',lnwd,'color','g')
    plot([kcMax kcMax],[0 1],'k','linewidth',lnwd/4);
    
    for k = 1:length(kc)
        plot(kc(k),snr(k),'bx','linewidth',lnwd / 4)
    end
    hold off
    % kcMax = kcMax/ (2 * 100/63/13);
    title(['Dcor analysis : res ~ ',num2str(kcMax ,4),', SNR ~ ',num2str(A0,4)])
    xlim([0 1]); ylim([0 1]); % 4712500 
    xticks([0 0.2 0.4 0.6 0.8 1]);
    % xticklabels({'0','0.94','1.89','2.83','3.77','4.71'});% [0 0.2 0.4 0.6 0.8 1] / (2 * 100/72.5/13)
    xticklabels({'0','0.82','1.64','2.46','3.28','4.09'});% [0 0.2 0.4 0.6 0.8 1] / (2 * 100/63/13)
    xlabel('Spatial frequencies (um-1)')
    ylabel('Cross-correlation coefficients')
    set(gca,'FontSize',30)
end
